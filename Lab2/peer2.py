...
# Defining peer
class peer:
    def __init__(self,hostAddr,peerId,neighbors,db,totalPeers,traders):
                ...
                if x > 0:
                    ...
                    for neighbor in self.neighbors:
                        if neighbor['peerId'] > self.peerId:
                            if self.trader != {} and neighbor['peerId'] == self.trader[0]:
                                pass
                            else:    
    ...
    def transaction(self, productName, sellerId, buyer_id,tradeCount):
        if self.db["Role"] == "Buyer":	
            ...
            if productName in self.db['shop']:	
                ...
                self.clockAdjust(otherClock,self.trader[0])
                with open('Peer_'+str(self.peerId)+".txt", "a") as f:
    ...
    if totalPeers<3:
        ...
        if buyerCnt<1 or sellerCnt<1:
            ...
            for peerId in range(1,totalPeers+1):
                ...
                #Start the thread 
                peer_local = peer(hostAddr,peerId,neighbors,db,totalPeers,traders)
                ...
                thread2.start() 
                #no need of election as trader already chosen
                # if peerId <= 2:
